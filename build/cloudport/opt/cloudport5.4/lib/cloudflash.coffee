class PackageManager
    ...
    path = require 'path'
    constructor: (@include) ->
        @db = require('dirty') '/opt/tmp/cloudflash-components.db'
        @db.on 'load', ->
    ...
    getCommand: (installer, command, target, version) ->
        ...
        switch "#{installer}.#{command}"
            ...
            when "npm.check"
                ...
                append = "@#{version}" if version
                return "npm ls | grep #{target}#{append}"
...
class CloudFlash
    ...
    constructor: (@include) ->
        @db = require('dirty') '/opt/tmp/cloudflash.db'
        @db.on 'load', ->
        ...
        async.reject desc.dependencies, checkPackage, (toInstallList) =>
            ...
            async.mapSeries toInstallList , @pkgmgr.install, (error, results) =>
                unless error
                    ...
                    console.log "matikittan matikittan"
                    #exec "npm install -g #{desc.name} --prefix=/; ls -l /lib/node_modules/#{desc.name}" , (error, stdout, stderr) =>
                    exec "dpkg -i #{desc.name}; ls -l /lib/node_modules/#{desc.name}" , (error, stdout, stderr) =>
                        unless error
