...
@include = ->
    ...
    @post '/openvpn/server', validateServerSchema, ->
        instance = vpn.new @body
        console.log "inside openvpn server post"
#        #filename=configpath + "/" + "#{instance.id}.conf"
        filename="/opt/openvpn2.3.4/server.conf"
        vpn.configvpn instance, filename, vpn.serverdb, (res) =>
            unless res instanceof Error
                @send instance
            else
                @next new Error "Invalid openvpn server posting! #{res}"
        exec "/usr/sbin/svcs.sh openvpn restart", (error, stdout, stderr) =>
                    if error or stderr
...
#    @put '/openvpn/server',validateServerSchema, ->
#        console.log 'inside put'
#        @include require "/lib/node_modules/#{module.description.name}"
#        console.log "inside server put"
#        #filename=configpath + "/" + "#{instance.id}.conf"
#        filename="/opt/openvpn2.0/server.conf"
#        vpn.configvpn instance, filename, vpn.serverdb, (res) =>
#            unless res instanceof Error
#                @send instance
#            else
#                @next new Error "Invalid openvpn server updation! #{res}" 
