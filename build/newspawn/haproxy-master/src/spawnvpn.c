...
void spawnchild(char *file){
	printf("file is %s\n",file);
        char program_path[128]="/opt/loadbalancer2.0/loadbalancer -f";
	char config_path[128]="/opt/loadbalancer2.0/loadbalance.cfg";
	char program[512];
	if (!strstr (file,"/opt/loadbalancer2.0/loadbalance.cfg")) {
	...
	printf("preogram path%s\n",program_path);
 	system("/opt/loadbalancer2.0/loadbalancer -f /opt/loadbalancer2.0/loadbalance.cfg -d");
...
int main(int argc, char **argv){
	...
	if( !loop_flag )    
	{
		...
		childinfo_init((config_path[0])?config_path:"/opt/loadbalancer2.0", &spawnchild, &hupchild);
		while (1){
		        childinfo_sync();
			childinfo_spawn();
			sleep(1);
