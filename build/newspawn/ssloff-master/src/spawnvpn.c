...
void spawnchild(char *file){
	...
	char program[512];
	if (!strstr (file,"/opt/ssloffloader2.0/ssloffload.cfg")) {
...
int check_spawnvpn_process( void )
{
	...
	stream = popen( "pgrep spawnssloff", "r" );
...
int main(int argc, char **argv){
	...
	if( !loop_flag )    
	{
		...
		childinfo_init((config_path[0])?config_path:"/opt/ssloffloader2.0", &spawnchild, &hupchild);
		while (1){
		        childinfo_sync();
			childinfo_spawn();
			sleep(1);
