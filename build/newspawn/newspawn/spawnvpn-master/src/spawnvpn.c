...
//	printf("file is %s\n",file);
        char program_path[128]="/opt/openvpn2.3.4/openvpn";
	char config_path[128]="/opt/openvpn2.3.4/server.conf";
	char program[512];
	if (!strstr (file,"/opt/openvpn2.3.4/server.conf")) {
	...
	printf("befor program path%d\n",program_path[0]);
		...
		printf("inside program_path\n");
            sprintf(program, "%s%s", program_path, "/opt/openvpn2.3.4/openvpn"); 
	    execl(program, "openvpn",  "--config", file, "--syslog", file, (char *)NULL);
        } else {
		printf("inside else\n");
	    execl("/opt/openvpn2.3.4/", "openvpn",  "--config", file, "--syslog", file, (char *)NULL);
        }
}
...
int main(int argc, char **argv){
	...
	if( !loop_flag )    
	{
		//printf("config paht%d\n",config_path[0]);
		//childinfo_init((config_path[0])?config_path:"/config/openvpn", &spawnchild, &hupchild);
		childinfo_init((config_path[0])?config_path:"/opt/openvpn2.3.4", &spawnchild, &hupchild);
		while (1){
		        childinfo_sync();
			childinfo_spawn();
			sleep(1);
